...
3. **左节点**释放，`_next` 析构，因 `_next` 析构，指向的**右节点**
4. **右节点**释放，`_prve` 析构，因 `_prve` 析构，指向**左节点**
- 逻辑上形成闭环，因为谁都不会释放，导致内存泄漏
- 把 **ListNode** 结构体中的 `_next 和 _prve` 从 `shared_ptr` 改成 `weak_ptr`，这是就不会增加引用计数，`_next 和 _prev` 不参与资源释放管理逻辑，就打破了循环应用

### weak_ptr（弱引用） 
- 不增加引用计数，用于打破 shared_ptr 的循环引用导致的内存泄漏。 - 不能直接解引用，必须用 `lock()` 转为 shared_ptr 使用。
```cpp
#include<iostream>
using namespace std;
template<class T>
...
**小结**  
RAII 是 C++ 异常安全的基石：资源与对象生命周期绑定，异常抛出时栈展开自动调用析构函数，保证资源释放。  
智能指针就是 RAII 的最佳实践。

后续我会更新 Linux 系列笔记，欢迎关注～
代码仓库：[https://github.com/yuuki233233/cpp-learning-journey](https://github.com/yuuki233233/cpp-learning-journey) 
CSDN 主页：[https://blog.csdn.net/yuuki233233](https://blog.csdn.net/yuuki233233)
