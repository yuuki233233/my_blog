...
>
>为解决这一问题，红黑树（Red-Black Tree）作为一种**近似平衡的二叉搜索树**应运而生 —— 它放弃了 AVL 树 “严格平衡” 的要求，转而通过「节点颜色标记 + 5 条核心规则」实现 “黑高一致” 的弱平衡，将任意根到叶子的路径长度差控制在 2 倍以内。这种设计让红黑树在保持 O(logN) 时间复杂度的同时，大幅降低了旋转频率（插入最多 2 次旋转，删除最多 3 次），成为工程实践中更优的选择（C++ STL 的`map/set`、Linux 内核 CFS 调度器、Java 的`TreeMap`等均基于红黑树实现）。
>
